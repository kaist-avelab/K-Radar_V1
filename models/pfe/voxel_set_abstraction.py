...
from utils.ops.pointnet2.pointnet2_stack import pointnet2_modules as pointnet2_stack_modules
from utils.ops.pointnet2.pointnet2_stack import pointnet2_utils as pointnet2_stack_utils
...
def sample_points_with_roi(rois, points, sample_radius_with_roi, num_max_points_of_part=400000):#200000):
    """
    Args:
        rois: (M, 7 + C)
        points: (N, 3)
    ...
    ### Add for point mask == 0 to avoid all batch samples 0###
    if point_mask.shape[0] > 0:
        point_mask[:1] = True
    ###########################################################
...
class VoxelSetAbstraction(nn.Module):
    def __init__(self, model_cfg, num_bev_features=None,
                 num_rawpoint_features=None, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg

        self.point_cloud_range = self.model_cfg.MODEL.VOXEL_ENCODER.LDR_PC_RANGE
        self.voxel_size = self.model_cfg.MODEL.VOXEL_ENCODER.LDR_VOXEL_SIZE
        
        SA_cfg = self.model_cfg.MODEL.PFE.SA_LAYER

        self.SA_layers = nn.ModuleList()
        self.SA_layer_names = []
        self.downsample_times_map = {}
        c_in = 0
        for src_name in self.model_cfg.MODEL.PFE.FEATURES_SOURCE:
        ...
        if 'bev' in self.model_cfg.MODEL.PFE.FEATURES_SOURCE:
            c_bev = self.model_cfg.MODEL.PFE.NUM_BEV_FEATURES
            c_in += c_bev

        if 'raw_points' in self.model_cfg.MODEL.PFE.FEATURES_SOURCE:
            num_rawpoint_features = self.model_cfg.MODEL.VOXEL_ENCODER.NUM_POINT_FEATURES
        ...
        c_in = 544 # I'm not sure where the additional c comes from (should be 288). Gonna check later.
        
        self.vsa_point_feature_fusion = nn.Sequential(
            nn.Linear(c_in, self.model_cfg.MODEL.PFE.NUM_OUTPUT_FEATURES, bias=False),
            nn.BatchNorm1d(self.model_cfg.MODEL.PFE.NUM_OUTPUT_FEATURES),
            nn.ReLU(),
        )
        self.num_point_features = self.model_cfg.MODEL.PFE.NUM_OUTPUT_FEATURES
        self.num_point_features_before_fusion = c_in
        self.model_cfg.MODEL.PFE.NUM_FEATURES_BEFORE_FUSION = c_in
    ...
    def sectorized_proposal_centric_sampling(self, roi_boxes, points):
        ...
        sampled_points, _ = sample_points_with_roi(
            ...
            sample_radius_with_roi=self.model_cfg.MODEL.PFE.SPC_SAMPLING.SAMPLE_RADIUS_WITH_ROI,
            num_max_points_of_part=self.model_cfg.MODEL.PFE.SPC_SAMPLING.get('NUM_POINTS_OF_EACH_SAMPLE_PART', 200000)
        )
        sampled_points = sector_fps(
            points=sampled_points, num_sampled_points=self.model_cfg.MODEL.PFE.NUM_KEYPOINTS,
            num_sectors=self.model_cfg.MODEL.PFE.SPC_SAMPLING.NUM_SECTORS
        )
        return sampled_points
    ...
    def get_sampled_points(self, batch_dict):
        ...
        if self.model_cfg.MODEL.PFE.POINT_SOURCE == 'raw_points':
            src_points = batch_dict['points'][:, 1:4]
            batch_indices = batch_dict['points'][:, 0].long()
        elif self.model_cfg.MODEL.PFE.POINT_SOURCE == 'voxel_centers':
        ...
        for bs_idx in range(batch_size):
            ...
            if self.model_cfg.MODEL.PFE.SAMPLE_METHOD == 'FPS':
                cur_pt_idxs = pointnet2_stack_utils.farthest_point_sample(
                    sampled_points[:, :, 0:3].contiguous(), self.model_cfg.MODEL.PFE.NUM_KEYPOINTS
                ).long()

                if sampled_points.shape[1] < self.model_cfg.MODEL.PFE.NUM_KEYPOINTS:
                    times = int(self.model_cfg.MODEL.PFE.NUM_KEYPOINTS / sampled_points.shape[1]) + 1
                    non_empty = cur_pt_idxs[0, :sampled_points.shape[1]]
                    cur_pt_idxs[0] = non_empty.repeat(times)[:self.model_cfg.MODEL.PFE.NUM_KEYPOINTS]

                keypoints = sampled_points[0][cur_pt_idxs[0]].unsqueeze(dim=0)

            elif self.model_cfg.MODEL.PFE.SAMPLE_METHOD == 'SPC':
    ...
    def aggregate_keypoint_features_from_one_source(
        ...
        if filter_neighbors_with_roi:
            ...
            for bs_idx in range(batch_size):
                ...
                # print('batch points: ', xyz[bs_mask].shape, 'valid points: ', point_features[bs_mask][valid_mask].shape)
    ...
    def forward(self, batch_dict):
        ...
        if 'raw_points' in self.model_cfg.MODEL.PFE.FEATURES_SOURCE:
            ...
            # print('\n', 'raw_points', torch.unique(raw_points[:, 0]))
            ...
            pooled_features = self.aggregate_keypoint_features_from_one_source(
                ...
                filter_neighbors_with_roi=self.model_cfg.MODEL.PFE.SA_LAYER['raw_points'].get('FILTER_NEIGHBOR_WITH_ROI', False),
                radius_of_neighbor=self.model_cfg.MODEL.PFE.SA_LAYER['raw_points'].get('RADIUS_OF_NEIGHBOR_WITH_ROI', None),
        ...
        for k, src_name in enumerate(self.SA_layer_names):
            ...
            # print('\n', src_name, torch.unique(cur_coords[:, 0]))
            ...
            pooled_features = self.aggregate_keypoint_features_from_one_source(
                ...
                new_xyz=new_xyz, new_xyz_batch_cnt=new_xyz_batch_cnt,
                filter_neighbors_with_roi=self.model_cfg.MODEL.PFE.SA_LAYER[src_name].get('FILTER_NEIGHBOR_WITH_ROI', False),
                radius_of_neighbor=self.model_cfg.MODEL.PFE.SA_LAYER[src_name].get('RADIUS_OF_NEIGHBOR_WITH_ROI', None),
                rois=batch_dict.get('rois', None)
            )
