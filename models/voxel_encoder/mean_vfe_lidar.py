...
class MeanVoxelEncoderLidar(nn.Module):
    def __init__(self, cfg):
        ...
        for i in range(len(io_channels)-1):
            self.voxel_encoder.append(nn.Sequential(
                ...
                nn.BatchNorm1d(io_channels[i+1]),
                nn.ReLU()
            ))
    ...
    def forward(self, data_dic, **kwargs):
        ...
        for batch_id in range(data_dic['batch_size']):
            pc = ldr_pc_64[torch.where(pts_batch_indices == batch_id)].view(-1, 4) # N x 4 (x, y, z, i)
        ...
        # Change ldr_pc_64 name to points
