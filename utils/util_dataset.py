...
def func_show_radar_tensor_bev(p_pline, dict_item, bboxes=None, \
    ...
    if not (bboxes is None):
        arr_yx_bbox = draw_bbox_in_yx_bgr(arr_yx, arr_y, arr_x, bboxes)
...
def func_show_radar_cube_bev(p_pline, dict_item, bboxes=None, magnifying=4, is_with_doppler = False, is_with_log = False):
    rdr_cube, rdr_cube_mask, rdr_cube_cnt = p_pline.get_cube(dict_item['meta']['path_rdr_cube'], mode=0)
    if is_with_doppler:
        rdr_cube_doppler = p_pline.get_cube_doppler(dict_item['meta']['path_cube_doppler'])
    print(bboxes)
    ...
    normalizing = p_pline.cfg.DATASET.RDR_CUBE.NORMALIZING.METHOD
    if normalizing == 'max':
